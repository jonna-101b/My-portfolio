        ...
        return (
                <div className="wrapper" onClick={(e) => { e.stopPropagation() }}>
                        <div className="container">
                                ...
                                <div className="tags">
                                        { article.tags.map((tag, index) => (<p key={index} style={{ color: colors[index % colors.length] }}>{ tag }</p>)) }
                                </div>
                                ...
                                <div className="links">
                                        { article.links.map((link, index) => (<button key={index} >{ link.title }</button>)) }
                                </div>
                        </div>
                </div>
        );
...
function View() {
        ...
        return (
                <div className={`view ${isEmptyObject(view) ?  "" : "active" }`} onClick={handleDisplay} >
                        { isEmptyObject(view) ? "" : <Wrapper article={view} /> }      
                </div>
        );
}
